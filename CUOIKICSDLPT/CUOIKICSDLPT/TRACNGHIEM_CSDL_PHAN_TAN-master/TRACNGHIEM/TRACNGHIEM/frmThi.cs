...
namespace TRACNGHIEM
{
    public partial class frmThi : Form
    {
        ...
        private void insertdiemsv()
            ...
            {
            sqlConvertDate = ngayThi.ToString("yyyy-MM-dd HH:mm:ss.fff");
            /*String sql = "INSERT INTO dbo.BANGDIEM(MABD, MASV, MAMH, LAN, NGAYTHI, DIEM, BAITHI) VALUES (" +
                MABD + ", '" + Program.mSV + "', '" + edtMaMon.Text + "', " + cbbLanThi.SelectedValue.ToString() + ", '" + sqlConvertDate + "', " + diem + " , 'TRAC NGHIEM')";*/
            //MessageBox.Show(sql);
            String sql = "INSERT INTO dbo.BANGDIEM(MASV, MAMH, LAN, NGAYTHI, DIEM, BAITHI, MABD) VALUES ('" 
                 + Program.mSV + "', '" + edtMaMon.Text + "', " + cbbLanThi.SelectedValue.ToString() + ", '"
                + sqlConvertDate + "', " + diem + ", 'TRACNGHIEM' ," + MABD.ToString().Trim() + ")";
        ...
        public void getMABD()
        {
            String sql = "SELECT MAX(MABD) FROM LINK0.TracNghiem.dbo.BANGDIEM ";
            Program.myReader = Program.ExecSqlDataReader(sql);
            if (Program.myReader == null)
            {
                Program.myReader.Close();
            ...
            {
                Program.myReader.Read();
                MABD = Program.myReader.GetInt32(0) + 1;
                Program.myReader.Close();
                Program.conn.Close();
            }
        }
        ...
        private void ghiDapAn()
        {
            ...
            for (int i = 0; i < listCauHoi.Length; i++)  
            {
                /*sqlInsert = "insert into dbo.BAITHI(CauSo, NoiDung, DaChon, MABD ) VALUES " +
                    "(" + (i + 1) + ", N'" + listCauHoi[i].NDCauHoi + "', '" + listCauHoi[i].CauDaChon + "', " + MABD + ")";*/
                sqlInsert = "insert into Baithi(CauSo, NoiDung, A, B, C, D, DapAn,DaChon , MaBD) values(" +
                   listCauHoi[i].CauSo + ",N'" + listCauHoi[i].NDCauHoi + "',N'" + listCauHoi[i].CauA + "',N'" + listCauHoi[i].CauB + "',N'" + listCauHoi[i].CauC + "',N'" + listCauHoi[i].CauD + "','" +
                    listCauHoi[i].CauDapAn + "','"+ listCauHoi[i].CauDaChon + "','"+ MABD.ToString().Trim() +"')";
               /* Console.WriteLine(sqlInsert);*/
                /*MessageBox.Show(sqlInsert);*/
                try
                {
                    int kq = Program.ExecSqlNonQuery(sqlInsert);
                    Program.conn.Close();
                }
                catch (Exception ex)
                {
